...
def get_session(session_id: str) -> Optional[Dict[str, Any]]:
    if db_manager.is_connected():
        return db_manager.get_interview_by_id(session_id)
    return LOCAL_SESSIONS.get(session_id)

def save_session(session_id: str, data: Dict[str, Any]) -> bool:
    if db_manager.is_connected():
        return db_manager.save_interview(data)
    LOCAL_SESSIONS[session_id] = data
    return True

def delete_session(session_id: str) -> bool:
    if db_manager.is_connected():
        return db_manager.delete_interview(session_id)
...
def list_sessions() -> List[Dict[str, Any]]:
    if db_manager.is_connected():
        return db_manager.get_all_interviews()
    ...
    for sid, data in LOCAL_SESSIONS.items():
        result.append({
            ...
            "created_at": sid # using session_id as fallback
        })
    # Sort reverse by session_id
...
async def delete_interview_detail(session_id: str):
